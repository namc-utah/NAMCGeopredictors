...
if (nrow(def_watersheds) == nrow(def_sites) & modelId %in% watershed_models) {
  message('all sites have sheds!')
} else {
  if (nrow(def_watersheds) != nrow(def_sites) & modelId %in% watershed_models) {
    message(paste(length(clipr::write_clip(def_sites$siteId[def_sites$siteId %in% def_watersheds$siteId == FALSE]))),
            ' sites need watersheds. Site names have been copied to your clipboard!')
  } else {
    message('no watersheds needed!')
  }
}
