...
big_download_dem_tiles <- function(outlet_coords, buffer_distance_m) {
  ...
  outlet_point <- sf::st_sfc(outlet_point, crs = 3338)
  ...
  dem_tiles <- elevatr::get_elev_raster(buff_poly, z = 12)
...
for(i in 1:nrow(x)){
  ...
  outlet=sf::st_transform(outlet,crs=3338)
  ...
  tryCatch(
    {
      ...
      r <- extract_river(outlet = c(outlet_df$longitude, outlet_df$latitude),
                         n_processes = 8,EPSG=3338,DEM = DEM_Raster)
