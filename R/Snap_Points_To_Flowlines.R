...
data = query("samples", boxId = 2806)
View(data) #View the data
lat <- data$sampleLatitude#extract just the latitude
lon <- data$sampleLongitude#extract just the longitude
df <- data.frame(lon, lat)#create a dataframe of the lat/longs
multipoint_matrix <- as.matrix(df)#create a matrix from the dataframe
...
nhd_proj <- st_transform(nhd, crs = 4326)#transform nhd to crs of the state layer so they are the same
...
df <- st_as_sf(x = data, coords = c("sampleLongitude", "sampleLatitude"), crs = 4326)
